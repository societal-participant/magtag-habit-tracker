...
Controls:
  ...
  Button D - short press: Toggle selected habit done/not-done for today
             long press (~1.5s): manually advance to a new day
                                  (fallback if you don't set up WiFi)
...
Library Bundle), copied into /lib:
  ...
  adafruit_magtag, adafruit_minimqtt, adafruit_portalbase,
  adafruit_connection_manager.mpy, adafruit_fakerequests.mpy, adafruit_il0373.mpy,
  adafruit_requests.mpy, adafruit_ticks.mpy, neopixel.mpy, simpleio.mpy
...
# ---------- Configuration ----------
HABITS = ["Take Vitamins", "Drink Water", "Exercise", "Read", "Stretch"]
...
def try_sync_time():
    """Attempt WiFi + NTP sync. Safe to call even with no secrets.py."""
    global time_synced
    try:
...
footer_label = label.Label(
    terminalio.FONT,
    text="A: Sync  B/C:Move  D:Toggle (hold D=NewDay)",
    color=BLACK,
    scale=1,
    anchor_point=(0, 0.5),
...
def show_daily_progress():
    """Each of the first 4 completed habits lights one more blue pixel.
    Completing the 5th (i.e. all habits done) turns all 4 pixels gold."""
    pixels = magtag.peripherals.neopixels
